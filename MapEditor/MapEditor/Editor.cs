...
namespace MapEditor {
    public partial class Editor : Form {
        ...
        private void btnLoad_Click(object sender, EventArgs e) {
                ...
                for (int i = 0; i < mapString.GetLength(0); i++) {
                    for (int j = 0; j < mapString.GetLength(1); j++) {
                        textureRotation[i,j] = input.ReadInt32();
                        for (int r = 0; r < textureRotation[i, j]; r++) { // stores it in the map array for editing
                ...
                for (int i = 0; i < mapString.GetLength(0); i++) {
                    for (int j = 0; j < mapString.GetLength(1); j++) {
                        objectRotation[i, j] = input.ReadInt32();
                        for (int r = 0; r < objectRotation[i, j]; r++) { // stores it in the map array for editing
                ...
                for (int x = 0; x < enemyWidth; x++) {
                    for (int y = 0; y < enemyHeight; y++) {
                        ...
                        } else if (entityString[x, y] == "RiotEnemy") {
                            Bitmap ent = new Bitmap("Entities/RiotEnemy.png"); // load object texture
                            entityMap[x, y] = ent; // stores it in the object array
                        }
                    }
                }
